...
function createSyncProtocol(p,m){
...
function validateBody(b){
 check(b&&Object.keys(b).length===9&&[1,2].includes(b.schema)&&id(b.vaultId)&&id(b.deviceId)&&id(b.mutationId)&&id(b.epoch));
 ...
 for(const op of b.operations){check(op&&Object.keys(op).length===4&&id(op.objectId)&&!objects.has(op.objectId)&&typeof op.deleted==="boolean");objects.add(op.objectId);check(decimal(op.baseVersion)<18446744073709551615n);
  check(Array.isArray(op.fields)&&op.fields.length<=256&&(op.deleted?(b.schema===2||op.fields.length===0):op.fields.length>0));const slots=new Set();
...
async function createBatch({state,epoch,deviceId,device,counter,mutationId,key,changes,schema=1}){
 const d=state.devices.get(deviceId);check(d?.role==="write"&&d.signingKey.equals(device.signing.publicKey));
 const body={schema,vaultId:state.vaultId,epoch,deviceId,membershipRevision:state.revision,keyGeneration:state.keyGeneration,counter,mutationId,operations:[]};
 ...
 for(const change of changes){const op={objectId:change.objectId,baseVersion:change.baseVersion,deleted:change.deleted,fields:[]};
  check(Array.isArray(change.fields)&&change.fields.length<=256&&(!change.deleted||schema===2||change.fields.length===0));
...
return {createBatch,verifyBatch,decryptBatch,verifyReceipt,decimal};
}
