...
async function applyMembership(state,record) {
 ...
 if(Object.keys(event).length!==6 || !Number.isSafeInteger(event.revision) || event.revision<1 || event.vaultId!==state.vaultId || event.revision!==state.revision+1 || event.previous!==state.head)
...
module.exports={verifyGenesis,applyMembership,applyRecovery,verifyMutation};
