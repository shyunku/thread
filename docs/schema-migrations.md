...
| DB schema migration version | DB에 적용한 순서 있는 구조 변경 | 서버 5, local root 1, legacy user 2, sync-v2 1 |
...
- schema 5는 sync_backfills 이관 기록을 추가한다. 전체 계정 데이터 이관은 [운영자 실행 도구](v2-rollout.md)로 별도 수행한다.
- 향후 schema 6 이상을 manifest 끝에 추가한다. 운영 데이터 이관·백업·배포는 사용자가 직접 수행한다.
