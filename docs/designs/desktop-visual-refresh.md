...
- 사이드바 접기 버튼은 Home 레이아웃에서 사이드바 오른쪽 안쪽 12px, top 26px에 배치한다. 기본 padding과 테두리 박스 없이 단일 왼쪽 화살표를 중앙에 표시한다. 접으면 같은 top 26px의 좌측 가장자리에 아이콘 없는 9×88px 핸들(클릭 영역 18×88px)을 남기고, hover/focus 시 강조한다. 키보드로도 다시 펼칠 수 있다. 프로필 영역은 78px에서 66px로 줄인다. 분할 보기의 정렬 옵션·추가 입력·목록은 우측 8px 여백을 공유한다.
...
핸들 후속 조정 검증: 2026-09-07 15:37 (KST)

- 접기/펼치기의 top 좌표가 같고, 접기 버튼 기본 padding이 0이며 단일 화살표가 중앙에 놓이는지 확인했다.
- 핸들 표시 폭 9px·클릭 폭 18px·높이 88px·아이콘 미표시와 펼치기 동작을 실제 브라우저로 확인했다. 기존 UI 검증도 통과했다.
