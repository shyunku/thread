...
- 현재 시간은 22px, 시계 아래 여백은 기존보다 10px 추가한다. 오늘/전체 및 카테고리 메뉴 버튼은 32px 높이다.
...
- 검색창 폭은 이전보다 30px 넓은 min(470px, 40vw + 30px)이다. 날짜 셀 경계는 개별 border 대신 격자 배경과 1px gap으로 그린다.
