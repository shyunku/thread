...
import (
	"fmt"
	"thread_api/libs/color"
	"thread_api/libs/console"
	ThreadJson "thread_api/libs/json"
	"thread_api/libs/math"
	ThreadStrings "thread_api/libs/strings"
	"regexp"
	"runtime"
	"strconv"
	"strings"
...
func (bl *BaseLogger) Write(p []byte) (n int, err error) {
	...
	bl.preComposeLog(NIL, ThreadJson.PrettyMarshal, false, str)
	return len(p), nil
}

func (bl *BaseLogger) Test(args ...any) {
	bl.preComposeLog(TEST, ThreadJson.PrettyMarshal, false, args...)
}

func (bl *BaseLogger) Debug(args ...any) {
	bl.preComposeLog(DEBUG, ThreadJson.PrettyMarshal, false, args...)
}

func (bl *BaseLogger) Info(args ...any) {
	bl.preComposeLog(INFO, ThreadJson.Marshal, false, args...)
}

func (bl *BaseLogger) Warn(args ...any) {
	bl.preComposeLog(WARN, ThreadJson.Marshal, false, args...)
}

func (bl *BaseLogger) Error(args ...any) {
	bl.preComposeLog(ERROR, ThreadJson.Marshal, false, args...)
}

func (bl *BaseLogger) Fatal(args ...any) {
	bl.preComposeLog(FATAL, ThreadJson.Marshal, false, args...)
}

func (bl *BaseLogger) Testf(format string, args ...any) {
	bl.preComposeLog(TEST, ThreadJson.PrettyMarshal, true, format, args)
}

func (bl *BaseLogger) Debugf(format string, args ...any) {
	bl.preComposeLog(DEBUG, ThreadJson.PrettyMarshal, true, format, args)
}

func (bl *BaseLogger) Infof(format string, args ...any) {
	bl.preComposeLog(INFO, ThreadJson.Marshal, true, format, args)
}

func (bl *BaseLogger) Warnf(format string, args ...any) {
	bl.preComposeLog(WARN, ThreadJson.Marshal, true, format, args)
}

func (bl *BaseLogger) Errorf(format string, args ...any) {
	bl.preComposeLog(ERROR, ThreadJson.Marshal, true, format, args)
}

func (bl *BaseLogger) Fatalf(format string, args ...any) {
	bl.preComposeLog(FATAL, ThreadJson.Marshal, true, format, args)
}

func (bl *BaseLogger) preComposeLog(level Level, marshalFunc ThreadJson.MarshalFunc, isFormatMode bool, args ...any) {
	if isFormatMode {
...
func (bl *BaseLogger) composeLog(level Level, marshalFunc ThreadJson.MarshalFunc, contents []any) {
	// Context Tag
	label := bl.label
	if label == "" {
		label = "GLOBAL"
...
func (bl *BaseLogger) buildLog(segments []string, contents []any, marshalFunc ThreadJson.MarshalFunc) string {
	stringBuilder := ThreadStrings.NewStringBuilder()
