create table thread.user_master
(
...
create table thread.admin_master
(
    uid varchar(255) null,
    constraint admin_master_user_master_uid_fk
        foreign key (uid) references thread.user_master (uid)
);

create table thread.transactions
(
    ...
    constraint transactions_user_master_uid_fk
        foreign key (`from`) references thread.user_master (uid)
);

create table thread.blocks
(
    uid             varchar(255) not null,
    ...
    constraint blocks_transactions_hash_fk
        foreign key (tx_hash) references thread.transactions (hash)
);
